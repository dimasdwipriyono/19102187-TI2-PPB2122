package com.dimasdwipriyono_19102187.praktikum7
        ...
        try {
            if (bundle != null) {
                ...
                for (aPdusObj in pdusObj) {
                    ...
                    val showSmsIntent = Intent(context, SmsReceiverActitvity::class.java)
                    showSmsIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                    showSmsIntent.putExtra(SmsReceiverActitvity.EXTRA_SMS_NO, senderNum)
                    showSmsIntent.putExtra(SmsReceiverActitvity.EXTRA_SMS_MESSAGE, message)
